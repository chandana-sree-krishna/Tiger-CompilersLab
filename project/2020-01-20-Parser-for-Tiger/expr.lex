...
<INITIAL> "<>"                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.NEQ (!lineNum, !prevCharNum) );  
<INITIAL> "<="                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.LE (!lineNum, !prevCharNum) );  
<INITIAL> ">="                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.GE (!lineNum, !prevCharNum) );  
<INITIAL> ":="                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
...
<INITIAL> "("                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.LP (!lineNum, !prevCharNum) );  
<INITIAL> ")"                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.RP (!lineNum, !prevCharNum) );  
<INITIAL> "{"                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.LFB (!lineNum, !prevCharNum) );  
<INITIAL> "}"                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.RFB (!lineNum, !prevCharNum) );  
<INITIAL> "["                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.LSB (!lineNum, !prevCharNum) );  
<INITIAL> "]"                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.RSB (!lineNum, !prevCharNum) ); 
<INITIAL> ":"                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
...
<INITIAL> "="                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.EQ (!lineNum, !prevCharNum) );  
<INITIAL> "*"                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.MUL (!lineNum, !prevCharNum) ); 
<INITIAL> "/"                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.DIV (!lineNum, !prevCharNum) ); 
<INITIAL> "<"                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.LT (!lineNum, !prevCharNum) ); 
<INITIAL> ">"                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.GT (!lineNum, !prevCharNum) ); 
<INITIAL> "&"                              =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
...
<INITIAL> {digits}+                         =>    ( YYBEGIN INITIAL;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    Tokens.CONST (valOf (Int.fromString yytext), !lineNum, !prevCharNum) );
<STRING> {alpha}+                                 =>    ( YYBEGIN INITIAL;
                                                    inString := 0;
                                                    prevCharNum := !charNum;
                                                    charNum := !charNum + size yytext;
                                                    str := !str ^ yytext;
